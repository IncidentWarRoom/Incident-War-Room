...
func TestSetSeverity(t *testing.T) {
	...
	t.Run("updates the active incident's severity", func(t *testing.T) {
		...
		if _, err := svc.CreateIncident(ctx, 200, "DB is down", incident.SeverityLow, nil, "alice"); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}

		inc, err := svc.SetSeverity(ctx, 200, incident.SeverityHigh)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("invalid severity is rejected", func(t *testing.T) {
		svc, _, _ := newTestService()
		if _, err := svc.CreateIncident(ctx, 201, "outage", incident.SeverityLow, nil, "alice"); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}

		_, err := svc.SetSeverity(ctx, 201, incident.Severity("CRITICAL"))
	...
	t.Run("no active incident", func(t *testing.T) {
		...
		_, err := svc.SetSeverity(ctx, 999, incident.SeverityHigh)
